...
export default function BookingManagement() {
    ...
    const [confirmModal, setConfirmModal] = useState({ isOpen: false, id: null, details: '' });
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(collection(db, 'bookings'), (snapshot) => {
            ...
            // Sort by Date (newest first) helper using ISO string comparison or similar
            bookingsList.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
    ...
    const handleCancelClick = (booking) => {
        setConfirmModal({
            isOpen: true,
            id: booking.id,
            details: `${booking.courtName} on ${booking.date} (${booking.timeSlot}) by ${booking.userName || 'User'}`
        });
    };
    ...
    const confirmCancel = async () => {
        ...
        setConfirmModal({ isOpen: false, id: null, details: '' });
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <ConfirmationModal
                ...
                message={`Are you sure you want to cancel the booking for ${confirmModal.details}? This action cannot be undone.`}
                confirmText="Proceed"
            />
        </div>
    );
