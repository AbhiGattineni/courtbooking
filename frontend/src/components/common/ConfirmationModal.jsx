...
export default function ConfirmationModal({ isOpen, onClose, onConfirm, title, message, createBookingMode = false, confirmText, isDanger = true }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all scale-100 p-6 relative">
                ...
                ${createBookingMode ? 'bg-blue-600 hover:bg-blue-700' : isDanger ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}
              `}
                        >
                            {confirmText || (createBookingMode ? 'Confirm' : 'Delete')}
                        </button>
                    </div>
                </div>
            </div>
