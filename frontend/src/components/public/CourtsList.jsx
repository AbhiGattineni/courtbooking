...
export default function CourtsList() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (selectedSport === 'all' && types.length > 0) setSelectedSport(types[0]);
  ...
  useEffect(() => {
    if (courts.length > 0) {
      ...
      if (selectedSport === 'all') {
        ...
        setFilteredCourts(courts.filter(court => (court.sportType || 'other') === selectedSport));
      }
    }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Select date and sport to find available slots</p>
        </div>
        ...
        {filteredCourts.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 text-lg">No courts available for this sport.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-8">
            {filteredCourts.map(court => (
              <CourtCard key={court.id} court={court} selectedDate={selectedDate} />
            ))}
          </div>
        )}
      </div>
